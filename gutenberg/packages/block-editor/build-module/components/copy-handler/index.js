...
function useNotifyCopy() {
  ...
  return useCallback(function (eventType, selectedBlockClientIds) {
    ...
    if (selectedBlockClientIds.length === 1) {
      ...
      notice = eventType === 'copy' ? sprintf( // Translators: Number of blocks being copied
      __('Copied %d blocks to clipboard.'), selectedBlockClientIds.length) : sprintf( // Translators: Number of blocks being cut
      __('Moved %d blocks to clipboard.'), selectedBlockClientIds.length);
    }
...
function CopyHandler(_ref) {
  ...
  var handler = function handler(event) {
    ...
    if (!hasMultiSelection()) {
      ...
      var hasSelection = event.type === 'copy' || event.type === 'cut' ? documentHasTextSelection() : documentHasSelection(); // Let native copy behaviour take over in input fields.
